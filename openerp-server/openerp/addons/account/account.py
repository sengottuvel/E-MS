...
#				if can_compute:
	...
	_columns = {
		...
		'code': fields.char('Code', size=4, required=True, select=1),
		...
		'u_id': fields.many2one('res.users', 'Created By', readonly=True),
		'ap_user_id': fields.many2one('res.users', 'Approved By', readonly=True),
		'approv_date': fields.datetime('Approved Date', readonly=True),	
	...
	_defaults = {
		...
		'reconcile': False,
		'modify': 'no',
		'u_id' : lambda obj, cr, uid, context: uid,
		'active': True,
	...
	def entry_approve(self,cr,uid,ids,context=None):
		b = datetime.now()		
		d_time = b.strftime('%m/%d/%Y %H:%M:%S')		
		self.write(cr, uid, ids, {'state': 'approved','ap_user_id': uid, 'approv_date': d_time})
		return True
	...
	_sql_constraints = [
		...
		('name', 'unique (name)', 'The Tax Name must be unique !')
	]
...
class account_tax(osv.osv):
	...
	_columns = {
		...
		'type': fields.selection( [('percent','Percentage'), ('fixed','Fixed Amount'), ('none','None'), ('code','Python Code'), ('balance','Balance')], 'Tax Type', required=True,
		 states={'waiting':[('readonly',False)]},help="The computation method for the tax amount."),
		...
		'account_collected_id':fields.many2one('account.account', 'Invoice Tax Account',states={'waiting':[('readonly',False)]}, help="Set the account that will be set by default on invoice tax lines for invoices. Leave empty to use the expense account."),
		'account_paid_id':fields.many2one('account.account', 'Refund Tax Account', states={'waiting':[('readonly',False)]},help="Set the account that will be set by default on invoice tax lines for refunds. Leave empty to use the expense account."),
		...
		'child_depend':fields.boolean('Tax on Children', states={'waiting':[('readonly',False)]},help="Set if the tax computation is based on the computation of child taxes rather than on the total amount."),
...
class ac_account_tax(osv.osv):
	_name = 'account.tax'
	_inherit = 'account.tax'
	_description = 'Tax'
	
	_columns = {
		'use_id': fields.many2one('res.users', 'Created By', readonly=True),
		'app_user_id': fields.many2one('res.users', 'Approved By', readonly=True),
		'approve_date': fields.datetime('Approved Date', readonly=True),
	}
	
	_defaults = {
		'use_id' : lambda obj, cr, uid, context: uid,
	}
