...
instance.web.CrashManager = instance.web.Class.extend({
    ...
    show_warning: function(error) {
        ...
        /***** Start Ticket 3116 */
        //toastr.error(error.data.fault_code, error.type);
		setTimeout(function () {
			swal(error.type, error.data.fault_code, "error");
		}, 100);
		// function removed
		/***** End Ticket 3116 */
    },
    ...
    do_login: function (db, login, password) {
        ...
        return this.session.session_authenticate(db, login, password).then(function() {
            ...
            self.$(".oe_login_pane").fadeIn("fast", function() {
				...
				toastr.error('Invalid Username or Password', "Warning!");				
            });
        });
    },
...
instance.web.Menu =  instance.web.Widget.extend({
    ...
    start: function() {
		...
		var e = h-71;
		$('.oe_secondary_menus_container').slimScroll({
            height: e+'px',
            alwaysVisible: false
        });
    ...
    open_menu: function (id) {
		...
		$('.oe_leftbar .oe_active').closest('ul').prev().css('border-bottom','0px solid');
    },
    /**
