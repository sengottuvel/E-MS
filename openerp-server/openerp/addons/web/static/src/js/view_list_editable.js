...
openerp.web.list_editable = function (instance) {
    ...
    instance.web.ListView.include(/** @lends instance.web.ListView# */{
        ...
        sort_by_column: function (e) {
			/***** Start Ticket 3002 */
            $(this.$el.find('table.oe_list_content')).stickyTableHeaders('destroy');
            $(this.$el.find('table.oe_list_content')).stickyTableHeaders();
        ...
        load_list: function (data, grouped) {
			...
			/***** Start Ticket 3002 */
			th = this.$el.find('table.oe_list_content thead th')
			$.each(th, function (ky, val) {
				if(ky > 0){
					$(val).text($(val).find("div").text());
				}
            });
            $('div.oe_list.oe_view table.oe_list_content').stickyTableHeaders({marginTop:32});
			setTimeout(function(){
				if(window.location.href.indexOf("view_type=list") > -1 == false) {				
				   self.$el.addClass('sticky_header');
				}
				
				if((window.location.href.indexOf("action=394") > -1 && window.location.href.indexOf("view_type=form") > -1) || 
					(window.location.href.indexOf("action=395") > -1 && window.location.href.indexOf("view_type=form") > -1) ||
					(window.location.href.indexOf("action=427") > -1 && window.location.href.indexOf("view_type=form") > -1)
				) {
				   $('body').find('thead.tableFloatingHeaderOriginal').addClass('action_394')
				}
			}, 500);
			setTimeout(function(){					
				if(window.location.href.indexOf("view_type=form") > -1) {
				   self.$el.removeClass('sticky_header');
				}
			}, 800);
			/***** End Ticket 3002 */
            ...
            if (this.editable()) {
				...
				/***** Start Ticket 3002 */
                //$('div.oe_list.oe_view.oe_list_editable table.oe_list_content').stickyTableHeaders('destroy');
                $('div.oe_list.oe_view.oe_list_editable table.oe_list_content').stickyTableHeaders();
				/***** End Ticket 3002 */
